...
<div className='form-row'>
<div className='form-group col-md-12'>
  <label >Birthdate: <b>{Formatter().dateToString(this.state.birthdate)}</b></label>
</div>
</div>
...
{ this.state.typeId === 1?
 <div className="form-row">
     <div className='form-group col-md-12'><label>Salary: {NumFormat().commaTwoDecimalString(this.state.salary)} </label></div>
     <div className='form-group col-md-12'><label>Tax: 12% </label></div>
</div> : <div className="form-row">
<div className='form-group col-md-12'><label>Rate Per Day: {NumFormat().commaTwoDecimalString(this.state.salary)} </label></div>
</div> }
...
<div className="form-row">
<div className='form-group col-md-12'>
  <label>Net Income: <b>{NumFormat().commaTwoDecimalString(this.state.netIncome)}</b></label>
</div>
</div>
...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async calculateSalary() {
    ...
    const requestOptions = {
        ...
        body: JSON.stringify({id: this.state.id, days: this.state.days})
    };
    const response = await fetch('api/employees/' + this.state.id + '/calculate',requestOptions);
    const data = await response.json();
    this.setState({ loadingCalculate: false,netIncome: data, days: this.state.days >= 0 ? this.state.days : 0 });
  }
